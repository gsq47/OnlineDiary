...
app.get('/',checkLogin.noLogin);//通过使用checkLogin.noLogin函数将对于‘/’的请求路由到指定路径，
//即如果未登陆不可直接访问首页
...
app.post('/register',function(req,res){//获得在/register页面提交的数据，并执行以下函数
	...
	User.findOne({username:username},function(err,user){//在数据库存放User文档的部分查找是否存在输入的username
		...
		return res.redirect('/register');//重新刷新该页面
       	...
       	if(user){//如果查找到了
		error+="The username is exist !";//错误信息+1 提示用户名已经存在
...
app.get('/weixinui',function(req,res){//自己测试微信
...
app.get('/notelist',function(req,res){//自己测试notelist
...
app.listen(3000,function(req,res){//监听 3000端口
